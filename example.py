...
def select_bands(img):
    ...
    # return img.expression('b("B6","B5","B4")*10000')
...
def neuron(inputs, w, bias):
    ...
    output = potential.expression('1.0 / (1 + exp(-potential))', {'potential': potential.select('sum')})
    return output


def nnet(input_1, input_2, input_3, input_4, input_5, input_6):
    hidden_pot_1=-3.44450377987436 +input_1*(74.3718551556474)+input_2*(-21.2367519758289)+input_3*(-55.7135867952326)+input_4*(13.8055979723598)+input_5*(33.8263553699906)+input_6*(-4.65603663256619)
    hidden_1 = 1/(1+exp(-hidden_pot_1))
    hidden_pot_2=1.74165858322656 +input_1*(-12.9459938873083)+input_2*(34.9694205850822)+input_3*(-5.16519568791497)+input_4*(-21.7194850261224)+input_5*(-17.3280785945837)+input_6*(71.0147521320201)
    hidden_2 = 1/(1+exp(-hidden_pot_2))
    hidden_pot_3=-0.933320762369686 +input_1*(-25.6815062078177)+input_2*(19.0405107028956)+input_3*(30.5258581643624)+input_4*(-45.6688597695154)+input_5*(3.50747732974286)+input_6*(58.4352978956202)
    hidden_3 = 1/(1+exp(-hidden_pot_3))
    out_pot1=21.4929274624373 +hidden_1*(45.9907843593617)+hidden_2*(-29.351969547187)+hidden_3*(44.488091219665)
    out_neuron_1 = 1/(1+exp(-out_pot1))

    return out_neuron_1

def main(geom_mask):
    mod5_alarms15ig = ee.FeatureCollection('ft:1nSY5gPlvffkZH5-rS02hY5mNw07WwFT7JJ5dYnXn');
    ...
    # print data.expression('b("B4")/10000').getInfo()



    # get sample var
    #sample = data.sampleRegions(collection=train_cut, properties=['Class'], scale=30)

    # seed = 2015;
    # classifier = ee.Classifier.randomForest(numberOfTrees=500, seed=seed);
    # print classifier
    ...
    task = ee.batch.Export.image(
        classified2015b,
        description='classified_2015_adjusted1',
        config=task_config
    )
